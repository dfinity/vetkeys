...
fn should_preserve_state_across_upgrade() {
    // Runs against both backends via the shared harness: the Rust canister
    // (generated by `export_encrypted_maps_canister!`, state in stable
    // structures) and the Motoko canister (the `EncryptedMapsCanister` mixin,
    // state in main memory via enhanced orthogonal persistence). Both must
    // return the value inserted before the upgrade. `upgrade_example_canister`
    // picks the matching upgrade mode for the wasm under test.
    let rng = &mut reproducible_rng();
    let env = TestEnvironment::new(rng);
    let caller = random_self_authenticating_principal(rng);
    let map_name = random_map_name(rng);
...
fn upgrade_example_canister(env: &TestEnvironment) {
    let wasm = load_encrypted_maps_example_canister_wasm();
