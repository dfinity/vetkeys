...
fn should_preserve_state_across_upgrade() {
    // This test validates the `#[post_upgrade]` generated by the Rust
    // `export_encrypted_maps_canister!` macro. The shared harness is also run
    // against the Motoko canister wasm (via CUSTOM_WASM_PATH), which persists
    // state through a different mechanism (orthogonal persistence) and is
    // covered by its own PR; so scope this Rust-macro-specific test to the
    // default (Rust) wasm.
    if std::env::var("CUSTOM_WASM_PATH")
        .map(|p| !p.is_empty())
        .unwrap_or(false)
    {
        return;
