...
impl VrfOutput {
    ...
    fn compute_vrf_hash(vetkey: &VetKey, dpk: &DerivedPublicKey, vrf_input: &[u8]) -> [u8; Self::VRF_BYTES] {
        /*
         ...
         */
        let mut ro_input = Vec::with_capacity(Self::VRF_BYTES + G1AFFINE_BYTES + G2AFFINE_BYTES + vrf_input.len() + 4 * 8);
        extend_with_length_prefix(&mut ro_input, vetkey.serialize());
        extend_with_length_prefix(&mut ro_input, &dpk.serialize());
        extend_with_length_prefix(&mut ro_input, vrf_input);

        let mut vrf_output = [0u8; Self::VRF_BYTES];
    ...
    pub fn create(proof: &VetKey, input: &[u8], dpk: &DerivedPublicKey) -> Self {
        let vrf_output = Self::compute_vrf_hash(proof, dpk, input);
        Self {
            proof: proof.clone(),
            dpk: dpk.clone(),
            vrf_output,
    ...
    pub fn deserialize(bytes: &[u8]) -> Result<Self, InvalidVrfOutput> {
        ...
        let proof = VetKey::deserialize(&bytes[0..G1AFFINE_BYTES]).map_err(|_| InvalidVrfOutput::InvalidProof)?;

        let dpk = DerivedPublicKey::deserialize(&bytes[G1AFFINE_BYTES..G1AFFINE_BYTES + G2AFFINE_BYTES])
            .map_err(|_| InvalidVrfOutput::InvalidProof)?;

        let vrf_input = bytes[G1AFFINE_BYTES + G2AFFINE_BYTES..].to_vec();

        let vrf_output = Self::compute_vrf_hash(&proof, &dpk, &vrf_input);
