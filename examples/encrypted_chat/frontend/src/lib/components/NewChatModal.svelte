<script lang="ts">
  ...
  async function createDirect() {
    await chatActions.createDirectChat(directPrincipal, directRotationMinutes, directExpirationMinutes);
    close();
  }
  ...
  <div class="fixed inset-0 z-50 flex items-center justify-center bg-black/50 p-4" role="dialog" aria-modal="true">
    <Card class="w-full max-w-lg">
      ...
      <div class="mb-4 flex gap-2">
        <button class="btn {tab === 'direct' ? 'variant-filled-primary' : 'variant-ghost-surface'}" onclick={() => (tab = 'direct')}>
          <User class="h-4 w-4" />
          <span class="ml-1">Direct</span>
        </button>
        <button class="btn {tab === 'group' ? 'variant-filled-primary' : 'variant-ghost-surface'}" onclick={() => (tab = 'group')}>
          <Users class="h-4 w-4" />
      ...
      {#if tab === 'direct'}
        <div class="space-y-3">
          <label class="block text-sm">
            ...
            <input class="w-full rounded border px-3 py-2" bind:value={directPrincipal} placeholder="aaaaa-aa" />
          </label>
          <div class="grid grid-cols-2 gap-3">
            <label class="block text-sm">
              <span class="mb-1 block font-medium">Rotation (min)</span>
              <input type="number" min="0" class="w-full rounded border px-3 py-2" bind:value={directRotationMinutes} />
            </label>
            <label class="block text-sm">
              <span class="mb-1 block font-medium">Expiration (min)</span>
              <input type="number" min="0" class="w-full rounded border px-3 py-2" bind:value={directExpirationMinutes} />
            </label>
          </div>
      ...
      {:else}
        <div class="space-y-3">
          <label class="block text-sm">
            ...
            <textarea rows="3" class="w-full rounded border px-3 py-2" bind:value={groupPrincipalsText} placeholder="comma or space separated principals"></textarea>
          </label>
          <div class="grid grid-cols-2 gap-3">
            <label class="block text-sm">
              <span class="mb-1 block font-medium">Rotation (min)</span>
              <input type="number" min="0" class="w-full rounded border px-3 py-2" bind:value={groupRotationMinutes} />
            </label>
            <label class="block text-sm">
              <span class="mb-1 block font-medium">Expiration (min)</span>
              <input type="number" min="0" class="w-full rounded border px-3 py-2" bind:value={groupExpirationMinutes} />
            </label>
          </div>
          <div class="flex justify-end gap-2">
